...
function removeEnrollment($enrollment_id,$userRecord) {
	...
	$query = 	"SELECT groups.id FROM ".makeTableName("groups")." AS groups \n";
	...
	$query .=	"INNER JOIN ".makeTableName("group_membership")." AS member ON member.group_id = groups.id AND member.user_id = removerEnroll.user_id;";
	$result = $db->prepared_query($query,array($enrollment_id));
	if ( $result->num_rows ) {
		throw new ServerClientException(
