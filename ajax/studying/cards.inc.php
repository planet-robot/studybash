...
function validateCard(&$rec) {
	...
	if ( !$len || ( $len > 14336 ) ) {
		throw new Exception("Validation failed (question_text). Rec: ".print_r($rec,true));
	}
	...
	if ( !empty($rec['answer_text']) ) {
		...
		$len = strlen($rec['question_text']);
		if ( $len > 4096 ) {
			throw new Exception("Validation failed (answer_text). Rec: ".print_r($rec,true));
		}
	}
	...
	if ( count($rec['tags']) ) {
		...
		foreach ( $rec['tags'] as $tag ) {
			...
			if ( in_array($tag['id'],$tagIDs) ) {
				throw new Exception("Validation failed (tags:duplicate). Rec: ".print_r($rec,true));
			}
		...
		if ( $result->num_rows !== count($tagIDs) ) {
			throw new Exception("Validation failed (tags:not found). Rec: ".print_r($rec,true));
		}
	}
