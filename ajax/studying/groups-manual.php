...
try {	
	...
	if ( ( $request_method === "POST" ) && ( $num_parms === 2 ) && ( $parms[0] === "create" ) ) {
		...
		try {
			...
			$query =	"SELECT * FROM ".makeTableName("groups")." AS groups \n";
			$query .=	"INNER JOIN ".makeTableName("enrollment")." AS ownerEnroll ON ownerEnroll.module_id = ? AND ownerEnroll.id = groups.enrollment_id \n";
			...
			$query .=	"SELECT groups.id, users.id FROM ".makeTableName("groups")." AS groups \n";			
			$query .=	"INNER JOIN ".makeTableName("users")." AS users ON users.id = ? \n";
			$query .=	"INNER JOIN ".makeTableName("enrollment")." AS groupEnroll ON groupEnroll.id = groups.enrollment_id AND groupEnroll.module_id = ? \n";
	...
	// the entire group.
		...
		try {
		    ...
		    $query = 	"LOCK TABLES ".makeTableName("groups")." AS groups WRITE, ".makeTableName("group_membership")." AS members WRITE, \n";
		    ...
		    $query =	"SELECT groups.*, groupEnroll.user_id = ? AS is_user_owner, COUNT(members.id) AS num_members FROM ".makeTableName("groups")." AS groups \n";
		    $query .=	"INNER JOIN ".makeTableName("enrollment")." AS groupEnroll ON groupEnroll.id = groups.enrollment_id AND groupEnroll.module_id = ? \n";
		    $query .=	"LEFT JOIN ".makeTableName("group_membership")." AS members ON members.group_id = groups.id \n";		    
		    $query .=	"WHERE groups.id = ? AND groups.code = ? \n";
		    ...
		    $query .=	"INNER JOIN ".makeTableName("groups")." AS groups ON groups.id = members.group_id AND groups.id = ? \n";
		    ...
		    if ( $num_members === 1 ) {
		    	...
		    	$query =	"DELETE groups FROM ".makeTableName("groups")." AS groups \n";
		    	$query .=	"LEFT JOIN ".makeTableName("group_membership")." AS members ON members.group_id = groups.id \n";
			    $query .=	"WHERE groups.id = ? AND members.group_id IS NULL;";
		    ...
		    // we'll do this simply by picking the lowest ID of a user that's still in the group.
		    	...
		    	$query = 	"UPDATE ".makeTableName("groups")." AS groups \n";				
				$query .= 	"INNER JOIN ".makeTableName("enrollment")." AS ownerEnroll ON ownerEnroll.user_id = ? AND ownerEnroll.module_id = ? \n";
				$query .= 	"INNER JOIN ".makeTableName("group_membership")." AS members ON members.group_id = groups.id AND members.user_id = ownerEnroll.user_id \n";
				$query .=	"SET groups.enrollment_id = ownerEnroll.id \n";
				$query .=	"WHERE groups.id = ?;";
