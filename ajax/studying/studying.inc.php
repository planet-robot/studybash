...
function getSets($options) {
	...
	$query .=	"LEFT JOIN ( \n";
	$query .=	"	SELECT groups.id, groupEnroll.module_id, members.user_id FROM ".makeTableName("groups")." AS groups \n";
	$query .=	"	INNER JOIN ".makeTableName("enrollment")." AS groupEnroll ON groupEnroll.id = groups.enrollment_id ";
	$query .=	"	INNER JOIN ( \n";
	$query .=	"		SELECT * FROM ".makeTableName("group_membership")." \n";
	$query .=	"	) AS members ON members.group_id = groups.id \n";
...
function buildBreadcrumb($options) {
	...
	if ( array_key_exists("groupID",$options) ) {
		...
		if ( ( $options['groupID'] === "self" ) || ( $options['groupID'] === "pub" ) ) {
			...
			$query = 	"SELECT 'groupID' AS crumbName, groups.id, ownerUser.full_name AS created_by_full_name, ownerUser.last_name AS created_by_last_name, ownerUser.first_name AS created_by_first_name, \n";
			$query .=	"ownerUser.id AS created_by_id FROM ".makeTableName("groups")." AS groups \n";
			$query .=	"INNER JOIN ".makeTableName("enrollment")." AS ownerEnroll ON ownerEnroll.id = groups.enrollment_id \n";
			$query .=	"INNER JOIN ".makeTableName("users")." AS ownerUser ON ownerUser.id = ownerEnroll.user_id \n";
...
function getGroups($options) {
	...
	if ( $options['groupIDs'] === null ) {
		...
		$groupsQueryStr = "(groups.id IN (" . $groupsQueryStr . "))";
	}
	...
	$query = 	"SELECT groups.*, COUNT(members.id) AS num_members, SUM(accessorMember.id) IS NOT NULL AS is_user_member, \n";
	...
	$query .=	"	FROM ".makeTableName("groups")." AS groups \n";
	...
	$query .=	"INNER JOIN ".makeTableName("group_membership")." AS members ON members.group_id = groups.id \n";
	...
	$query .=	"HAVING (is_user_member = 1 OR groups.code = ?);";
	$result = Db::get_instance()->prepared_query($query,$queryAry);
