...
class View {
	...
	public function build() {
		...
		if ( !$this->html_page ) {
			...
			$head->assign_scripts( array( Bootstrap::get_setting("js_root_full")."sysop/errors/js/app.js" ) );
			$head->assign_styles( array( Bootstrap::get_setting("js_root_full")."sysop/errors/css/sysop.css" ) );

			// header/footer of content. no assignments here.
			
			$header = new ViewModel("Sysop_Errors_Header.tpl");
			$footer = new ViewModel("Sysop_Errors_Footer.tpl");
			...
			if ( $req->success ) {
				...
				foreach ( $req->rows as $error ) {
					
					$trace_html = "";
					$traceLines = preg_split("/\s+#[0-9]+\s*/", (" ".$error->trace));
					array_splice($traceLines,0,1);

					for ( $x=0; $x < count($traceLines); $x++ ) {
						$tv = new ViewModel("Sysop_Errors_Trace.tpl");
						$tv->assign("num",count($traceLines)-$x-1);
						$tv->assign("text",$traceLines[$x]);
						$trace_html .= ( $tv->render(false)."\n");
					}

					$ev = new ViewModel("Sysop_Errors_Error.tpl");					
					$ev->assign("error",$error);
					$ev->assign("trace",$trace_html);
					$errors_html .= ( $ev->render(false)."\n" );
				}
			}

			$content = new ViewModel("Sysop_Errors_Content.tpl");		
			$content->assign("errors",$errors_html);
